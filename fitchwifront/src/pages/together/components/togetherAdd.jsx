...
const TogetherAdd = ({ facilitieList, refreshTogetherList, refreshTogetherTagList }) => {
  ...
  const getNodayList = useCallback((facilitiesCode) => {
    axios
      .get("/getNodayList", { params: { facilitiesCode: facilitiesCode } })
      .then((res) => {
        setNoday(res.data);
      })
  ...
  const addTag = useCallback(
    (e) => {
      ...
      setInsertForm({ ...insertForm, togetherTagContent: "" });
    },
        ...
        <Box bgcolor="white" sx={{ mb: 5 }} component="form" onSubmit={sendTogether}>
          ...
          <UserBox>
            {insertForm.memberEmail.memberSaveimg && (
              <Avatar
                src={`${insertForm.memberEmail.memberSaveimg}`}
            ...
            <Grid item xs>
              <Box component="div" sx={{ mt: 10, height: 230 }}>
                {insertForm.facilitiesCode.facilitiesCode === 0 ? (
                  <Typography component="div">시설 이용안함</Typography>
                ) : (
                  <Stack
                    style={{ height: 190 }}
                    component="div"
                  ...
                  >
                    <Box>🏢 시설명 : {insertForm.facilitiesCode.facilitiesName}</Box>
                    <Box>🗾 시설 위치 : {insertForm.facilitiesCode.facilitiesPosition}</Box>
                    <Box>💲 시설 1인 이용료 : {insertForm.facilitiesCode.facilitiesPrice}원</Box>
                    <Box>✅ 제휴 상태 : {insertForm.facilitiesCode.facilitiesGrade}</Box>
                    <Box>🧑 시설담당자 : {insertForm.facilitiesCode.facilitiesManager}</Box>
                    <Box>📞 시설담당자연락처 : {insertForm.facilitiesCode.facilitiesPhone}</Box>
                  </Stack>
                )}
              </Box>
            </Grid>
          ...
          {insertForm.togetherType === "선착순" ? null : (
            <TextField
              fullWidth
          ...
          <TextField
            ...
            required
          />
          <Stack direction="row" sx={{ mt: 3, height: 55 }} spacing={3}>
            <TextField
              fullWidth
              label="태그"
